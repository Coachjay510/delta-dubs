...
import { useStore, TEAMS, RATES } from '../hooks/useStore'
...
export default function Players() {
  ...
  const { isPlayer, isVolunteer, canSeeFinancials, teamAccess, filterByTeam, canEdit } = usePermissions()
  ...
  const readOnly = isPlayer || isVolunteer

  const filtered = useMemo(() => {
    const q = search.toLowerCase()
    // Players only see their own team
    let list = filterByTeam(players)
  ...
  }, [players, search, teamF, statusF, genderF, filterByTeam])
  ...
  return (
    <div style={{ padding: 24 }}>
          ...
          <table>
            <thead>
              <tr>
                <th>Player</th>
                {!isPlayer && <th>Team</th>}
                {!isPlayer && <th>Status</th>}
                <th>#</th>
                <th>Age</th>
                {!readOnly && <th>Parent</th>}
                {!readOnly && <th>Phone</th>}
            ...
            <tbody>
              ...
              {filtered.map(p => (
                <tr key={p.id}>
                  <td>
                    <div style={{ display:'flex', alignItems:'center', gap:8, cursor: readOnly ? 'default' : 'pointer' }}
                      onClick={() => !readOnly && setShowDetail(p)}>
                      <div className={`avatar ${AV[p.team] || 'av-drive'}`}>{initials(p.name)}</div>
                      <div>
                        <div style={{ fontWeight:600, fontSize:13 }}>{p.name}</div>
                        {!readOnly && p.playerEmail && <div style={{ fontSize:11, color:'var(--text3)' }}>{p.playerEmail}</div>}
                      </div>
                    </div>
                  </td>
                  {!isPlayer && <td><span style={{ fontSize:11, fontWeight:700, color: TEAMS.find(t=>t.id===p.team)?.color||'var(--text2)' }}>{p.team}</span></td>}
                  {!isPlayer && <td>{statusBadge(p.status)}</td>}
                  <td className="td-mono" style={{ color:'var(--text3)' }}>#{p.num || '—'}</td>
                  <td className="td-muted">{p.age || '—'}</td>
                  {!readOnly && <td style={{ fontSize:12 }}>{p.parent || '—'}</td>}
                  {!readOnly && <td className="td-mono" style={{ fontSize:11 }}>{p.phone || '—'}</td>}
      ...
      {/* Add/Edit Modal — only for non-read-only users */}
      {showModal && !readOnly && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal" style={{ width: 680 }}>
            ...
            <div className="modal-body">
              <div style={{ marginBottom:14, fontFamily:'var(--font-mono)', fontSize:10, color:'var(--np-green2)', letterSpacing:'2px' }}>// IDENTITY</div>
              <div className="form-grid" style={{ marginBottom:14 }}>
                {inp('name', 'Full Name', 'text', 'First Last')}
                {sel('team', 'Team', TEAMS.map(t => t.id))}
              ...
              <div style={{ marginBottom:14, fontFamily:'var(--font-mono)', fontSize:10, color:'var(--np-green2)', letterSpacing:'2px' }}>// CONTACT</div>
              <div className="form-grid" style={{ marginBottom:14 }}>
              ...
              <div style={{ marginBottom:14, fontFamily:'var(--font-mono)', fontSize:10, color:'var(--np-green2)', letterSpacing:'2px' }}>// FEES</div>
              <div className="form-grid" style={{ marginBottom:14 }}>
                <div className="form-group">
              ...
              <div style={{ marginBottom:14, fontFamily:'var(--font-mono)', fontSize:10, color:'var(--np-green2)', letterSpacing:'2px' }}>// RECRUITING</div>
              <div className="form-grid">
                {inp('colleges', 'College Interests', 'text', 'UCLA, USC…')}
                <div className="form-group full">
                  <label className="form-label">Notes</label>
                  <textarea className="form-textarea" value={form.notes || ''} onChange={e => setField('notes', e.target.value)} placeholder="Any notes…" />
                </div>
              </div>
            </div>
          ...
          <div className="modal" style={{ width: 680 }}>
            ...
            <div className="modal-body">
              <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:24 }}>
                ...
                <div>
                  <div style={{ fontFamily:'var(--font-display)', fontSize:26 }}>{showDetail.name}</div>
                  <div style={{ fontSize:13, color:'var(--text2)', marginTop:2 }}>
                    {showDetail.team} · #{showDetail.num || '—'} · {showDetail.ageGroup || showDetail.age || '—'}
                  </div>
                </div>
                <div style={{ marginLeft:'auto', display:'flex', gap:8 }}>
                  {canSeeFinancials && payBadge(showDetail)}
            ...
            <tbody>
              ...
              {filtered.map(p => (
                <tr key={p.id}>
                  ...
                  <td>
                    <span style={{ fontSize:11, fontWeight:700, color: TEAMS.find(t=>t.id===p.team)?.color || 'var(--text2)' }}>
                      {p.team}
                    </span>
                  </td>
